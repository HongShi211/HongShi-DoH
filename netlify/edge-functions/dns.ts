// Netlify Edge Function - Robust DoH + /ip
const DEFAULT_DOH='cloudflare-dns.com';
const JSON_HEADERS={'accept':'application/dns-json','user-agent':'HongShi-DoH/edge'} as const;
const BIN_HEADERS={'accept':'application/dns-message','user-agent':'HongShi-DoH/edge'} as const;
...
const enrichFromIpApi= async (ip:string)=>{const r=await fetch(`https://ipapi.co/${encodeURIComponent(ip)}/json/`,{headers:{'user-agent':'HongShi-DoH/edge'}}); if(!r.ok) return null; const j:any=await r.json(); const asnNum = parseInt(String(j.asn||'').replace(/[^\d]/g,''))||undefined; const out:any={
...
async function fetchJsonChain(urls:string[]){
  let lastErr:any=null;
  for(const u of urls){
    try{
      const r = await fetch(u,{headers: JSON_HEADERS, redirect:'follow'});
      const text = await r.text();
      if(r.ok){ return {ok:true, text}; }
      lastErr = text || r.statusText;
...
export default async (request: Request, context: any) => {
  ...
  if (p==='/resolve') {
    ...
    try {
      if (!doh || doh.startsWith(origin)) {
        ...
        if (out.ok) return new Response(out.text, {headers: cors(new Headers({'content-type':'application/json'}))});
        return new Response(JSON.stringify({error:'upstream failed', detail: out.err}), {status:502, headers: cors(new Headers({'content-type':'application/json'}))});
      }
      const h = (new URL(doh)).hostname;
      const chain = urlListForHost(h, name, type);
      const out = await fetchJsonChain(chain);
      if (out.ok) return new Response(out.text, {headers: cors(new Headers({'content-type':'application/json'}))});
      return new Response(JSON.stringify({error:'upstream error', detail: out.err}), {status:502, headers: cors(new Headers({'content-type':'application/json'}))});
    } catch (e:any) {
      return new Response(JSON.stringify({error: String(e)}), {status:502, headers: cors(new Headers({'content-type':'application/json'}))});
    }
  }

  // dohPath (binary / JSON passthrough)
  if (p===dohPath) {
    ...
    if (u.searchParams.has('name')) {
      const host = dohHost(env);
      const chain = urlListForHost(host, u.searchParams.get('name')!, u.searchParams.get('type')||'A');
      const out = await fetchJsonChain(chain);
      if (out.ok) return new Response(out.text, {headers: cors(new Headers({'content-type':'application/json'}))});
    ...
    const init:any = isGet ? {headers: BIN_HEADERS} : {method:'POST', headers:{...BIN_HEADERS,'content-type':'application/dns-message'}, body: await request.arrayBuffer()};
  ...
  // block default /dns-query when DOH_PATH changed

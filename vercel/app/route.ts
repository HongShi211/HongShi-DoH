...
async function enrichFromIpApi(ip:string){const r=await fetch(`https://ipapi.co/${encodeURIComponent(ip)}/json/`); if(!r.ok) return null; const j:any=await r.json(); const asnNum = parseInt(String(j.asn||'').replace(/[^\d]/g,''))||undefined; return {
...
function tryGoogleResolve(host:string,name:string,type:string){const u=new URL(`https://${host}/resolve`);u.searchParams.set('name',name);u.searchParams.set('type',type);return u.toString();}
function tryCfResolve(host:string,name:string,type:string){const u=new URL(`https://${host}/dns-query`);u.searchParams.set('name',name);u.searchParams.set('type',type);u.searchParams.set('ct','application/dns-json');return u.toString();}
export async function GET(req: Request){
  ...
  if(path==='/resolve'){const name=url.searchParams.get('name'); const type=url.searchParams.get('type')||'A'; const doh=url.searchParams.get('doh')||''; if(!name)return new Response(JSON.stringify({error:'name required'}),{status:400,headers:cors(new Headers({'content-type':'application/json'}))}); const origin=`${url.protocol}//${url.host}`; if(!doh||doh.startsWith(origin)){const host=dohHost; const g=await fetch(tryGoogleResolve(host,name,type)); if(g.ok) return new Response(await g.text(),{headers:cors(new Headers({'content-type':'application/json'}))}); const c=await fetch(tryCfResolve(host,name,type)); if(c.ok) return new Response(await c.text(),{headers:cors(new Headers({'content-type':'application/json'}))}); return new Response(JSON.stringify({error:'upstream failed'}),{status:502,headers:cors(new Headers({'content-type':'application/json'}))}); } const h=(new URL(doh)).hostname.toLowerCase(); const pri=h.includes('dns.google')?await fetch(tryGoogleResolve(h,name,type)):await fetch(tryCfResolve(h,name,type)); if(pri.ok) return new Response(await pri.text(),{headers:cors(new Headers({'content-type':'application/json'}))}); const fb=h.includes('dns.google')?await fetch(tryCfResolve(h,name,type)):await fetch(tryGoogleResolve(h,name,type)); if(fb.ok) return new Response(await fb.text(),{headers:cors(new Headers({'content-type':'application/json'}))}); return new Response(JSON.stringify({error:'upstream error'}),{status:502,headers:cors(new Headers({'content-type':'application/json'}))});}

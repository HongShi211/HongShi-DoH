...
const env = {
  DOH: process.env.DOH,
  PATH: process.env.PATH || process.env.TOKEN || 'dns-query',
};
...
async function handleDoH(req: Request): Promise<Response> {
  ...
  if (url.searchParams.has('name')) {
    const r = await fetch(jsonDoH + url.search, { headers: { 'accept':'application/dns-json' }});
  ...
  const init: RequestInit = isGetDns ? { headers: { 'accept':'application/dns-message' } } : {
    method:'POST', headers: { 'accept':'application/dns-message', 'content-type':'application/dns-message' }, body: await (req as any).arrayBuffer()
  };
...
export async function GET(req: NextRequest) {
  const url = new URL(req.url);
  if (url.searchParams.has('dns') || url.searchParams.has('name')) {
    return handleDoH(req);
  }
  return new Response('', { status:302, headers: { Location: '/ui' }});
}

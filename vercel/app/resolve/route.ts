...
export async function GET(req: Request) {
  try{
    ...
    let target = doh || (process.env.DOH ? `https://${process.env.DOH.replace(/^https?:\/\//,'').split('/')[0]}/resolve` : 'https://cloudflare-dns.com/resolve');
    const u = new URL(target);
    if (u.pathname.endsWith('/dns-query')) u.pathname = '/resolve';
    u.searchParams.set('name', name);
    u.searchParams.set('type', type);
    const r = await fetch(u.toString(), { headers: { 'accept':'application/dns-json','user-agent':'HongShi-DoH/edge' }});
